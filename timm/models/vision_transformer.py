""" Vision eransformer (ViT) in PyTorch
...
def scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False, scale=None, return_attention=False, return_cross_attention=False, weight_apply=False, fixed_tokens=None, old_att=None) -> torch.Tensor:
    # Efficient implementation equivalent to the following:
    L, S = query.size(-2), key.size(-2)
    scale_factor = 1 / math.sqrt(query.size(-1)) if scale is None else scale
    attn_bias = torch.zeros(len(query), 1, L, S, dtype=query.dtype).to(query.device)
    if is_causal:
    ...
    if attn_mask is not None:
        attn_mask = attn_mask.view(len(attn_mask), 1, 1, -1)
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_attention=False, return_cross_attention=False, weight_apply=False, fixed_tokens=None, old_att=None) -> torch.Tensor:
        B, N, C = x.shape
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, drop_tokens=0, return_attention=False, return_cross_attention=False, drop_option='drop', indices=None, weight_apply=False, return_features=False, fixed_tokens=None, old_att=None) -> torch.Tensor:
        att = self.attn(self.norm1(x), return_attention=return_attention, return_cross_attention=return_cross_attention,weight_apply=weight_apply, fixed_tokens=fixed_tokens, old_att=old_att)
        if return_attention:
        ...
        if return_attention:
            if return_features:
                ...
                return x, attn_weight, cross_attn_weight, normed_catt_weight, indices, attn_logit, None
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.gen_patch_embed = lambda x : embed_layer(
            img_size=img_size,
        ...
        self.patch_embed = self.gen_patch_embed(None)
    ...
    def add_multi_tokenizer(self, router_dim=1):
        self.alt_patch_embed = self.gen_patch_embed(None)
        self.router = nn.Conv2d(3, router_dim, 16, stride=16)

        # move to gpu
        self.alt_patch_embed = self.alt_patch_embed.cuda()
        self.router = self.router.cuda()


    def multi_patch_embed(self, x):
        with torch.no_grad():
            x_original = self.patch_embed(x)
        x_alt = self.alt_patch_embed(x)
        r = self.router(x)
        # use REINFORCE algorithm for routing like bandit problem.
        if r.shape[1] == 1:
            r = r.sigmoid()
            hard = r > 0.5
            ratio = hard + r - r.detach()
            log_prob = None
#            ratio = F.gumbel_softmax(r, tau=1, hard=True)
        else:
            r = r.view(len(r), r.shape[1], -1).permute(0, 2, 1)
            r = F.softmax(r, dim=-1)
            m = Categorical(r)
            ratio = m.sample()
            log_prob = m.log_prob(ratio)
        ratio = ratio.view(len(ratio), -1, 1)
        return ratio * x_original + (1-ratio) * x_alt, log_prob


    def forward_patch(self, x: torch.Tensor) -> torch.Tensor:
        """
            Forward pass through patch embedding - Jaedong
        """
        if hasattr(self, 'alt_patch_embed'):
            x, r = self.multi_patch_embed(x)
        else:
            x = self.patch_embed(x)
            r = None
        x = self._pos_embed(x)
    ...
    def get_ent_dist(self): 
        ent_distances = []
        for blk in self.blocks:
            ent_distances.append(blk.attn.dist_recording)
            blk.attn.dist_recording = []
        ent_distances = torch.Tensor(ent_distances)
        return ent_distances.mean(1)

    
    def forward_features(self, x: torch.Tensor, return_attention=False, return_cross_attention=False, skip_patch=False) -> torch.Tensor:
        if not skip_patch:
            if hasattr(self, 'prompter'):
                x = self.prompter(x)
            x, log_prob = self.forward_patch(x)
        if self.grad_checkpointing and not torch.jit.is_scripting():
            x = checkpoint_seq(self.blocks, x)
        else:
            attentions = []
            cross_attentions = []
            normed_cross_attentions = []
            attention_logits = []
            indices = []
            for i, blk in enumerate(self.blocks):
                x = blk(x, return_attention=return_attention, return_cross_attention=return_cross_attention)
                if return_attention:
                    x, attention, cross_attention, normed, idx, attention_logit, features = x
                    attentions.append(attention)
                    cross_attentions.append(cross_attention)
                    normed_cross_attentions.append(normed)
                    indices.append(idx)
                    attention_logits.append(attention_logit)
        x = self.norm(x)
        if return_attention:
            return x, attentions, cross_attentions, normed_cross_attentions, indices, attention_logits, log_prob
        return x
    ...
    def forward_head(self, x: torch.Tensor, pre_logits: bool = False) -> torch.Tensor:
        ...
        if not pre_logits:
            x = self.head(x)
            if self.zeroshot_weight is not None:
                # normalize weights
                x = x / x.norm(dim=-1, keepdim=True)
                x = 100 * x @ self.zeroshot_weight
        return x
